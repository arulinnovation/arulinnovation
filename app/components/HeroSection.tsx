...
const HeroSection = () => {
  ...
  return (
    ...
    >
      ...
      {/* Enhanced Scroll Indicator - Mobile Friendly */}
      <div 
        className="absolute bottom-3 xs:bottom-5 sm:bottom-10 left-1/2 transform -translate-x-1/2 animate-bounce"
        style={{ transform: `translateX(-50%) translateY(${Math.sin(scrollY * 0.01) * 3}px)` }}
      >
        <div className="w-6 h-9 xs:w-7 xs:h-10 sm:w-10 sm:h-15 border-2 border-white/70 rounded-full flex justify-center bg-gradient-to-b from-transparent to-white/25 backdrop-blur-md shadow-lg">
          <div className="w-1.5 h-3 xs:w-2 xs:h-4 sm:w-3 sm:h-6 bg-gradient-to-b from-white to-cyan-200 rounded-full mt-1.5 xs:mt-2 sm:mt-3 animate-pulse shadow-lg shadow-white/50"></div>
        </div>
      </div>
    </section>
  );
